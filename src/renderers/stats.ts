...
import { svgWrapper, sectionTitle, text, progressBar, formatNumber, statBlock } from '../svg/utils';
...
export function renderStats(s: StatsData): string {
  ...
  const grid = items.map((item, i) => {
    ...
    ${text(x + 12, y + 16, item.value, { size: 16, weight: '700', fill: item.color })}
    ${text(x + 12, y + 30, item.label, { size: 9, fill: THEME.textMuted })}`;
  }).join('');

  const inner = `
  ${sectionTitle(P, titleY, 'GitHub Stats')}
  ${grid}`;

  return svgWrapper(W, H, inner, 'GitHub Stats');
}
