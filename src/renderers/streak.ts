...
export function renderStreak(s: StreakData): string {
  ...
  const grid = blocks.map((b, i) => {
    ...
    ${text(cx, blockY + 32, b.big, { size: 28, weight: '700', fill: b.color, anchor: 'middle' })}
    ${text(cx, blockY + 50, b.label, { size: 11, fill: THEME.text, anchor: 'middle' })}
    ${text(cx, blockY + 66, b.sub, { size: 9, fill: THEME.textMuted, anchor: 'middle' })}`;
  }).join('');
  ...
  return svgWrapper(W, H, inner, 'Contribution Streak');
}
