  ...
  try {
    switch (path) {
      ...
      case '/icon': {
        ...
        const _op = parseFloat(params.get('opacity') ?? '');
        const opacity = Number.isNaN(_op) ? 0.2 : _op;
        const svg = renderIcon(name, color, parseInt(params.get('size') ?? '', 10) || 16, params.get('circle') === '1', opacity);
